...
class MatMulOp(Op):
    ...
    def compute(self, node, input_vals):
        ...
        assert len(input_vals) == 2
        mut_A = input_vals[0].transpose(-1, -2) if node.matmul_attr_trans_A else input_vals[0]
        mut_B = input_vals[1].transpose(-1, -2) if node.matmul_attr_trans_B else input_vals[1]
        return np.matmul(mut_A, mut_B)
...
def gradients(output_node, node_list):
    ...
    for node in reverse_topo_order:
        ...
        if isinstance(node.op, PlaceholderOp):
            continue
        input_grads = zip(node.inputs, node.op.gradient(node, grad))
        for k, v in input_grads:
            if k in node_to_output_grads_list.keys():
                node_to_output_grads_list[k].append(v)
            else:
                node_to_output_grads_list[k] = [v]
