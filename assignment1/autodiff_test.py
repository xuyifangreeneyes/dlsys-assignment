...
def test_add_mul_mix_2():
    ...
    x3_val = 3 * np.ones(3)
    x4_val = 4 * np.ones(3)
    y_val, grad_x1_val, grad_x2_val, grad_x3_val, grad_x4_val = \
        executor.run(feed_dict={x1: x1_val, x2: x2_val, x3: x3_val, x4: x4_val})
...
def test_add_mul_mix_3():
    ...
    expected_y_val = z_val * z_val + x3_val
...
def test_grad_of_grad():
    ...
    expected_y_val = x2_val * x2_val + x2_val * x3_val
...
def test_matmul_two_vars():
    ...
    expected_y_val = np.matmul(x2_val, x3_val)
    expected_grad_x2_val = np.matmul(np.ones_like(expected_y_val), np.transpose(x3_val))
    expected_grad_x3_val = np.matmul(np.transpose(x2_val), np.ones_like(expected_y_val))
